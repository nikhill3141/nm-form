...
import { CheckCircle2, Home, Leaf, Loader2, Lock, PenTool, Send, Star } from "lucide-react";
...
export function SharedFormRuntime() {
  ...
  const formQuery = trpc.formLink.getFormByLinkSlug.useQuery(
    { slug: routeSlug ?? "", formPassword: formPassword || undefined },
    { enabled: Boolean(routeSlug) && !isDraftPreview, retry: false },
  );
  const form = formQuery.data?.form;
  const requiresPassword = Boolean(formQuery.data?.requiresPassword);
  const hasPasswordAccess = !requiresPassword || Boolean(formQuery.data?.accessGranted);
  ...
  const fieldsQuery = trpc.field.getFieldsByFormId.useQuery(
    { formId, linkSlug: routeSlug, formPassword: formPassword || undefined },
    { enabled: Boolean(formId) && !isDraftPreview && hasPasswordAccess, retry: false },
  );
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-5rem)] max-w-5xl flex-col justify-between">
        ...
        {requiresPassword && !hasPasswordAccess ? (
          <section className="grid flex-1 place-items-center py-16">
            <form
              className={`w-full max-w-lg rounded-xl border p-6 shadow-2xl backdrop-blur-xl ${theme.panelClass}`}
              onSubmit={(event) => {
                event.preventDefault();
                setFormPassword(passwordInput);
              }}
            >
              <div className="mb-5 flex size-12 items-center justify-center rounded-lg bg-emerald-300 text-emerald-950">
                <Lock className="size-6" />
              </div>
              <p className="mb-3 text-sm font-semibold uppercase tracking-[0.24em] opacity-70">Protected form</p>
              <h1 className="text-3xl font-semibold leading-tight">Enter the form password</h1>
              <p className="mt-3 text-sm leading-6 opacity-76">
                This form is protected by its creator. Unlock it to view questions and submit a response.
              </p>
              <input
                className="nm-input mt-6"
                onChange={(event) => setPasswordInput(event.target.value)}
                placeholder="Form password"
                type="password"
                value={passwordInput}
              />
              <Button className={`${theme.buttonClass} mt-4 w-full`} disabled={formQuery.isFetching} type="submit">
                {formQuery.isFetching ? <Loader2 className="size-4 animate-spin" /> : <Lock className="size-4" />}
                Unlock form
              </Button>
              {formQuery.isError && (
                <p className="mt-3 rounded-lg border border-white/10 bg-white/10 p-3 text-sm opacity-80">
