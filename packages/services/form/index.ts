...
class FormService {
  ...
  public async createForm(userId: string, payload: CreateFormInputModelType) {
    ...
    const passwordSalt = formPassword ? randomBytes(16).toString("hex") : null;
    const passwordHash = formPassword && passwordSalt
      ? generateHash(passwordSalt, formPassword)
      : null;
  ...
  public async editForm(userId: string, payload: EditFormInputModelType) {
    ...
    const nextPasswordSalt = formPassword ? randomBytes(16).toString("hex") : null;
    const nextPasswordHash = formPassword && nextPasswordSalt
      ? generateHash(nextPasswordSalt, formPassword)
      : null;
    ...
    const updateForm = await db
      ...
      .set({
        ...
        ...(formPassword !== undefined && formPassword !== null && {
